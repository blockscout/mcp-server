...
async def test_get_transactions_by_address_calls_smart_pagination_correctly(mock_ctx):
    ...
    with (
        ...
        patch(
            "blockscout_mcp_server.tools.transaction_tools._fetch_filtered_transactions_with_smart_pagination", new_callable=AsyncMock
...
async def test_get_transactions_by_address_sparse_data_scenario(mock_ctx):
    ...
    for page_num in range(1, 6):  # 5 pages total
        ...
        page_responses.append({
            "items": page_items,
            "next_page_params": {"page": page_num + 1} if page_num < 5 else None
        })
    ...
    with (
        patch("blockscout_mcp_server.tools.transaction_tools.get_blockscout_base_url", new_callable=AsyncMock) as mock_get_url,
        patch("blockscout_mcp_server.tools.transaction_tools._fetch_filtered_transactions_with_smart_pagination", new_callable=AsyncMock) as mock_smart_pagination,
...
async def test_get_transactions_by_address_multi_page_progress_reporting(mock_ctx):
    ...
    mock_transactions = [
        {"type": "call", "hash": f"0x{i}", "block_number": 1000 - i, "from": "0xfrom", "to": "0xto"}
        for i in range(5)
    ]
    ...
    with (
        patch("blockscout_mcp_server.tools.transaction_tools.get_blockscout_base_url", new_callable=AsyncMock) as mock_get_url,
        patch("blockscout_mcp_server.tools.transaction_tools._fetch_filtered_transactions_with_smart_pagination", new_callable=AsyncMock) as mock_smart_pagination,
        patch("blockscout_mcp_server.tools.transaction_tools.report_and_log_progress", new_callable=AsyncMock) as mock_progress,
        patch.object(config, "advanced_filters_page_size", 10),
    ):
        mock_get_url.return_value = mock_base_url
