...
async def test_get_transactions_by_address_multi_page_fetching(mock_ctx):
    ...
    with (
        ...
        patch("blockscout_mcp_server.tools.transaction_tools.make_request_with_periodic_progress", new_callable=AsyncMock) as mock_request,
...
async def test_get_transactions_by_address_stops_at_10_pages(mock_ctx):
    ...
    with (
        patch("blockscout_mcp_server.tools.transaction_tools.get_blockscout_base_url", new_callable=AsyncMock) as mock_get_url,
        patch("blockscout_mcp_server.tools.transaction_tools.make_request_with_periodic_progress", new_callable=AsyncMock) as mock_request,
        patch.object(config, "advanced_filters_page_size", 20),  # Large page size to ensure we don't hit pagination limit
    ):
        mock_get_url.return_value = mock_base_url
        mock_request.side_effect = api_responses
...
async def test_get_transactions_by_address_single_page_sufficient(mock_ctx):
    ...
    page_items = []
    for i in range(15):
        page_items.append({"type": "call", "hash": f"0x{i+1}", "block_number": 100-i})
