...
The bundle doesn't include the original Python package. Instead, it uses [`mcp-remote`](https://github.com/geelen/mcp-remote) to proxy all requests to the [the official Blockscout MCP Server](http://mcp.blockscout.com/mcp).
...
3. Configure the Blockscout PRO API Key when prompted (see [Blockscout PRO API Key](../README.md#blockscout-pro-api-key))