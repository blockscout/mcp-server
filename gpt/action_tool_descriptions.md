<get_chains_list>
Get the list of supported blockchain chains with their IDs.
...
Supports POST requests with a JSON body (set `method` to "POST" and provide `json_body`) for endpoints like `/json-rpc`.
</direct_api_call>
